...
#define LOG_TRACE                                                                                  \
    ...
    Lux::Logger(__FILE__, __LINE__, Lux::Logger::LogLevel::TRACE, __func__).stream()
#define LOG_DEBUG                                                                                  \
    if (Lux::Logger::logLevel() <= Lux::Logger::LogLevel::DEBUG)                                   \
    Lux::Logger(__FILE__, __LINE__, Lux::Logger::LogLevel::DEBUG, __func__).stream()
#define LOG_INFO                                                                                   \
    if (Lux::Logger::logLevel() <= Lux::Logger::LogLevel::INFO)                                    \
    Lux::Logger(__FILE__, __LINE__).stream()
#define LOG_WARN Lux::Logger(__FILE__, __LINE__, Lux::Logger::LogLevel::WARN).stream()
#define LOG_ERROR Lux::Logger(__FILE__, __LINE__, Lux::Logger::LogLevel::ERROR).stream()
#define LOG_FATAL Lux::Logger(__FILE__, __LINE__, Lux::Logger::LogLevel::FATAL).stream()
...
#define CHECK_NOTNULL(val)                                                                         \
    ::Lux::CheckNotNull(__FILE__, __LINE__, "'" #val "' Must be non NULL", (val))
