/**
 * @file LuxINI.hpp
 * @brief 一个使用简单的 API 和最小的占用空间来操作 INI 文件。 它符合（有点）标准的 INI
 * 格式——部分和键不区分大小写，所有前导和尾随空格都被忽略。 注释是以分号开头的行。 节线上允许尾随注释。
 *
 * 文件按需读取，数据保存在内存中并关闭文件。 此实用程序支持惰性写入，它只将更改和更新写入文件并保留自定义格式和注释。
 * 由 write() 调用调用的惰性写入将读取输出文件，查找已进行的更改并相应地更新文件
...
namespace INIParser {
    ...
    inline void
    iniTransform(string& str) {}
#else // KEY_NOT_CASE_SENSITIVE
#ifdef KEY_LOWER
    inline void
    iniTransform(string& str) {
        std::transform(
            str.begin(), str.end(), str.begin(), [](const char c) { return static_cast<char>(std::tolower(c)); });
    }
#else // KEY_UPPER
    inline void
    iniTransform(string& str) {
        std::transform(
            str.begin(), str.end(), str.begin(), [](const char c) { return static_cast<char>(std::toupper(c)); });
    }
#endif
#endif
     ...
     */
    inline void
    replace(string& src, const string& target, const string& dst) {
        if (!target.empty()) {
            std::size_t pos = 0;
    ...
    public:
        ...
        enum class INILineType : char { PDATA_NONE, PDATA_COMMENT, PDATA_SECTION, PDATA_KEYVALUE, PDATA_UNKNOWN };
        bool isBOM_;
        bool prettyPrint_;
    ...
    private:
        /// `CORE` - Parse INI line
        inline INILineType
        parseLine(string line, ParseValues& parseData) {
        ...
        getFileContent() {
            ...
            fileReadStream_.read(&fileContents[0], static_cast<std::streamsize>(fileSize_));
            fileReadStream_.close();
        ...
        getLineData() {
            ...
            if (fileSize_ >= 3) {
                const char header[3] = {static_cast<char>(fileReadStream_.get()),
                                        ...
                                        static_cast<char>(fileReadStream_.get())};
                isBOM_ = (header[0] == static_cast<char>(0xEF) && header[1] == static_cast<char>(0xBB) &&
        ...
        inline bool
        writeLines2File(bool fileIsBOM, LineData output) {
            std::ofstream fileWriteStream(fileName2Write_, std::ios::out | std::ios::binary);
            if (fileWriteStream.is_open()) {
                if (fileIsBOM) {
                    fileWriteStream.write(INIParser::utf8_BOM, sizeof(INIParser::utf8_BOM));
                }
        ...
        ///
        LineData
        getLazyOutput(const LineDataPtr& lineData, INIStructure& data, INIStructure& original) {
            LineData output;
                        ...
                        if (data.has(sectionCurrent)) {
                            ...
                            if (collection.has(key)) {
                                ...
                                if (value == outputValue) {
                                    ...
                                    replace(lineNorm, "\\=", "  ");
                                    auto equalsAt = lineNorm.find_first_of('=');
                                    auto valueAt = lineNorm.find_first_not_of(whitespaceDelimiters, equalsAt + 1);
                ...
                if (writeNewKeys || std::next(line) == lineData->end()) {
                    ...
                    if (data.has(sectionCurrent) && original.has(sectionCurrent)) {
                        ...
                        for (const auto& it : collection) {
                            ...
                            replace(key, "=", "\\=");
                            trim(value);
                            linesToAdd.emplace_back(key + ((prettyPrint_) ? " = " : "=") + value);
                        }
                    }
                    if (!linesToAdd.empty()) {
                        output.insert(
                            output.begin() + static_cast<long>(lastKeyLine), linesToAdd.begin(), linesToAdd.end());
                    }
                    if (writeNewKeys) {
                        writeNewKeys = false;
                        --line;
            ...
            for (const auto& it : data) {
                ...
                for (const auto& it2 : collection) {
                    ...
                    output.emplace_back(key + ((prettyPrint_) ? " = " : "=") + value);
                }
            }
            return output;
        }
         ...
         */
        bool
        setINIStructure(const INIStructure& data) {
            if (!data.size()) {
                return true;
            }

            fileWriteStream_.open(fileName2Write_, std::ios::out | std::ios::binary);
            ...
            for (;;) {
                ...
                if (collection.size()) {
                    ...
                    for (;;) {
                        ...
                        fileWriteStream_ << key << ((prettyPrint_) ? " = " : "=") << value;
         ...
         */
        bool
        setINIStructure(INIStructure& data) {
            struct stat buf;
         ...
         */
        bool
        getINIStructure(INIStructure& data) {
            ...
            for (const auto& line : lines) {
                ...
                } else if (inSection && parseResult == INILineType::PDATA_KEYVALUE) {
                    const string& key = parseData.first;
                    const string& value = parseData.second;
    ...
    public:
         ...
         */
        INI(const string& fileName2Read, const string& fileName2Write, bool pretty = true, bool keepLineData = false)
            : isBOM_(false),
              prettyPrint_(pretty),
        ...
        bool
        generate(const INIStructure& data, bool pretty = true) {
        ...
        bool
        write(INIStructure& data, bool pretty = true) {
            if (fileName2Write_.empty()) {
                return false;
            }
