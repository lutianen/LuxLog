...
namespace detail {
    ...
    /// @brief 解决问题：万一程序执行了fork(2),那么子进程会不会看到 stale 的缓存结果？
    /// 利用 pthread_atfork 注册一个回调函数，用于清空缓存的线程ID
    void afterFork() {
    ...
    struct ThreadData {
        ...
        void runThread() {
            *tid_ = Lux::CurrentThread::tid();
            ...
            Lux::CurrentThread::t_threadName = name_.empty() ? "LuxThread" : name_.c_str();
...
/// 这样只有在本线程第一次调用的时候才进行系统调用,以后都是直接从thread local缓存的线程id拿到结果
