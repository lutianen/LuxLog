...
public:
    ...
    void memberFunc2(const std::string& text) {
        printf("tid=%d, Foo::x_=%f, text=%s\n", Lux::CurrentThread::tid(), x_, text.c_str());
    }
...
int main() {
    ...
    Lux::Thread t2(std::bind(threadFunc2, 42), "thread for free function with argument");
    t2.start();
    printf("t2.tid=%d\n", t2.tid());
    t2.join();
    ...
    Lux::Thread t4(std::bind(&Foo::memberFunc2, std::ref(foo), std::string("Lux")));
    t4.start();
    t4.join();
